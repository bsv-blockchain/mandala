...
`HandleReceive` keeps the `peerpay:` link at 240pt with the bare identity key beneath and Copy / Share. The QR is **not** switched to a bare key — the link carries `&url=` (the payee's message-box host, `handle.ts`), and dropping it would regress shipped BSV plumbing for token interop.

**Amended 2026-09-15 (maintainer decision) — the link grammar names the money.** Two optional parameters are added to the app's BRC-125 form: `asset=<txid.vout>` (the token's genesis outpoint, the one identifier every asset already has) and `amount=<base units>`. `sats=` is the BSV selector and `amount=` de facto names a token, so it is only valid beside `asset=`; `asset=` alone is an open token request (the payer chooses the figure); a link that mixes `sats=` with either is refused rather than read in one money or the other (`parsePeerPayURI.ts`, `rails/index.ts`, `peerPayLinkFor`). With a token selected, Get paid → handle therefore emits `peerpay:<key>?asset=<outpoint>[&amount=<n>][&url=<host>]`, and the method row reads the same in either money. On the Pay side a pasted, scanned or deep-linked token link selects that asset and seeds `amount` in its base units; a `sats=` link selects BSV; a token this wallet does not hold adopts the recipient, seeds no figure, and says so — *"This request is for a token this wallet doesn't hold. You can still pay them in BSV."* The typed figure is unit-tagged in the form, so a change of money (picker, link, or an asset dropped because it is not held) blanks it rather than re-reading it in the other unit. This supersedes the earlier rule that a token-selected link carried no figure; the nearby session's `t` block is unchanged.
